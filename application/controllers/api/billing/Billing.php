...
class Billing extends Rest_lib
{
        ...
        try {
            ...
            if ($token_data->status) {
                ...
                if ($type == 'user' && $id) {
                    ...
                    if ($this->BillingModel->GetUserBilling($id)) {
                        $message = array(
                            'success' => array(
                                'message' => 'Record/s fetched successfully!',
                                'data' => $this->BillingModel->GetUserBilling($id)
                            )
                        );
    ...
    public function index_post()
    {
        $token = null;
        $headers = apache_request_headers();
        log_message("INFO", "entered post");
        if (isset($headers['Authorization'])) {
            $token = str_replace('Bearer ', '', trim($headers['Authorization']));
        }
        try {
            log_message("INFO", "entered try");
            $token_data = (object) $this->objOfJwt->DecodeToken($token);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($token_data->status) {
                    log_message("INFO", "entered after form validation");
                    $billing_data = array(
                    ...
                    log_message("INFO", "finished billingmodel ");
                    //////////////////////////// AUDIT TRAIL ////////////////////////////
