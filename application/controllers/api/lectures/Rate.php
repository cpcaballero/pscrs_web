    ...
    public function index_post()
    {
        ...
        try {
            ...
            if ($token_data->status) {
                ...
                $this->form_validation->set_rules('id', 'Lecture', 'required|strip_tags|trim');
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($this->LectureModel->GetLecture($id)) {
                        ...
                        $is_rated = $this->LectureModel->CheckIfRated($this->input->post('active_user'), $id);

                        if (!$is_rated) {
                            $this->LectureModel->RateLecture($data);
                            $message = array(
                                'success' => array(
                                    'status' => true,
                                    'message' => 'Lessons rated successfully!'
                                )
                            );
