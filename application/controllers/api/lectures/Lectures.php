...
class Lectures extends Rest_lib
{
    ...
    public function index_get($id = null)
    {
        ...
        try {
            ...
            if ($token_data->status) {
                ...
                if ($id) {
                    if ($this->LectureModel->GetLecture($id)) {
                        $message = array(
                            'success' => array(
                                'message' => 'Record/s fetched successfully!',
                                'data' => $this->LectureModel->GetLecture($id)
                            )
                        );
                    ...
                    if ($this->LectureModel->GetLectures()) {
                        $message = array(
                            'success' => array(
                                'message' => 'Record/s fetched successfully!',
                                'data' => $this->LectureModel->GetLectures()
                            )
                        );
    ...
    public function index_post()
    {
        ...
        try {
            ...
            $this->form_validation->set_rules('title', 'Title', 'required|strip_tags|max_length[255]|is_unique[lectures.title]|trim');
            $this->form_validation->set_rules('owner_user_id', 'Owner', 'required|strip_tags|trim');
            $this->form_validation->set_rules('description', 'Description', 'required|trim');
            $this->form_validation->set_rules('video_link', 'Video Link', 'required|is_unique[lectures.video_link]|strip_tags|trim');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (($token_data->role == "superadmin" || $token_data->role == "admin") && $token_data->status) {
                    ...
                    $this->LectureModel->CreateLecture($user_data);
                    ...
                    $audit_data = (object) array(
                        ...
                        'action' => "create lecture - " . $this->input->post('title'),
                    );
            ...
            if ($token_data->status) {
                ...
                if ($id) {
                    ...
                    if ($this->LectureModel->GetLecture($id)) {
                        ...
                        $this->LectureModel->DeleteLecture($id);

                        $message = array(
                            'success' => array(
                                'message' => 'Lecture/s deleted successfully!'
                            )
                        );
