    ...
    public function index_post()
    {
        ...
        try {
            ...
            if ($token_data->status) {
                ...
                $this->form_validation->set_rules('id', 'News', 'required|strip_tags|trim');
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($this->NewsModel->GetNews($id)) {
                        ...
                        $is_rated = $this->NewsModel->CheckIfRated($this->input->post('active_user'), $id);

                        if (!$is_rated) {
                            $this->NewsModel->RateNews($data);
                            $message = array(
                                'success' => array(
                                    'status' => true,
                            ...
                            $message = array(
                                'success' => array(
                                    'status' => false,
                                    'message' => 'You have already rated this news!'
                                )
                            );
